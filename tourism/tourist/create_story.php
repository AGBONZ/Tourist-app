...
if (isset($_GET['slug']) && $_GET['slug'] != "" ) {
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($cat_id) || empty($title) || empty($desc) || empty($country) || empty($state) || empty($city) || empty($features) || empty($tmp_dir)) {
		...
		if ($createStory) {
			...
			foreach ($tmp_dir as $i => $file) {
				...
				if ($newFile_type != 'image' || $newFile_type != 'video') {
					echo "<script>alert('Invalid File Type, Story Created without Image');</script>";
				} else {
