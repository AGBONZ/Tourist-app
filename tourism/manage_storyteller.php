...
<?php  
	if (isset($_GET['delete'])) {
		...
		if ($deleteUser) {
			echo "<script>alert('DELETED !!! ');window.location='manage_storyteller'</script>";
		} else{
			echo "<script>alert('Error Deleting User');window.location='manage_storyteller'</script>";
		}
	}
	...
	if (isset($_GET['disable'])) {
		...
		if ($updateUser) {
			echo "<script>alert('UPDATED !!! ');window.location='manage_storyteller'</script>";
		} else{
			echo "<script>alert('Error Updating User');window.location='manage_storyteller'</script>";
		}
	}
?>
		<div class="card">
		  <div class="card-body p-4">
			  <h5 class="card-title"> Story Tellers</h5>
			  <hr/>
							...
							<tr>
								...
								<td width="15%">
									<div class="d-flex order-actions text-center">
										<a href="add_storyteller?edit=<?php echo $cat['id']?>" class=""><i class='bx bxs-edit text-success'></i></a>
										...
										<a href="?delete=<?php echo $cat['id']?>" class="bg-danger ms-3" onclick="return confirm('ARE YOU SURE?')"><i class='bx bxs-trash text-white'></i></a>
									</div>
								</td>
							</tr>
