<?php 
	...
	if(isset($_GET['delete'])){
		...
		$delStory = $connect2db->prepare("DELETE FROM story WHERE id = ?");
		if ($delStory->execute([$story_id])) {
			echo "<script>alert('Story Deleted');window.location='all-story'</script>";
		}else{
			echo "<script>alert('Error Deleting');window.location='all-story'</script>";
		}
	}
...
<div class="card">
	...
	<div class="card-body">
		...
		<div class="d-flex align-items-center gap-2">
			...
			<a href="?delete=<?php echo $story_id?>" onclick="return confirm('ARE YOU SURE?')" class="btn btn-danger"><i class='bx bxs-trash' ></i>Delete</a>
		</div>
