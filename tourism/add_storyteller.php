		...
		if (empty($fname) || empty($lname) || empty($uname) || empty($pnum)) {
			...
			if ($validateUser->rowcount() > 0) {
				...
				if ($createUser->execute([$fname, $lname, $uname, $pnum, $password, $role, $status])) {
					...
					echo "<script>alert('Error Adding Story Teller')</script>";
				}
		...
		if (empty($fname) || empty($lname) || empty($uname) || empty($pnum)) {
			...
			if ($validateUser->rowcount() > 0) {
				...
				if ($updateUser->execute([$uname, $lname, $fname, $pnum, $user_id])) {
					echo "<script>alert('Data Successfully Updated');window.location='manage_storyteller'</script>";
				} else {
		...
		<div class="card border-top border-0 border-4 border-primary">
			<div class="card-body p-5">
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary uppercase">Add Story Teller</h5>
				</div>
				<hr>
